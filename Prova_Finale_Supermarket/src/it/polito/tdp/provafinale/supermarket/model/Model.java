...
public class Model {
	...
	//metodi ricorsivi 
	                             //L è il numero di supermarket che si intende aprire
	public List<ComuneCandidato> espandi(int L, String ragione, double perc){
		...
		for(String regione : this.getRegioni()) {
			...
			Map<Comune, ComuneCandidato> mappaCandidati = daoC.getComuniCandidati(regione, ragione, mappaComuni, perc);
	...
	private void cerca(List<ComuneCandidato> parziale, int l, Map<Comune, ComuneCandidato> mappaCandidati, int lMax) {
        ...
        private void cercaZeroPresenti(List<ComuneCandidato> parziale, int l, Map<Comune, ComuneCandidato> mappaCandidati, int lMax) {
		
		
		if(l==lMax) 
		 return;
		
		 //Ad ogni passo aggiungo il migliore possibile, se è presente un candidato
		 if(!mappaCandidati.isEmpty()) {
		 
	     ComuneCandidato elemento = cercaMassimo(mappaCandidati);
		 parziale.add(elemento);
		 mappaCandidati.remove(elemento.getComune(),elemento);
		 
		 cercaZeroPresenti(parziale,l+1,mappaCandidati,lMax);
     ...
     */
	private ComuneCandidato cercaMassimo(Map<Comune,ComuneCandidato> mappaCandidati) {
		...
		mappaCandidati.put(cc.getComune(), cc);
      ...
      public List<ComuneCandidato> espandiZeroPresenti(int L, String ragione, double perc){
  		...
  		for(String regione : this.getRegioni()) {
  			...
  			Map<Comune, ComuneCandidato> mappaCandidati = daoC.getComuniCandidatiZeroPresenti(regione, ragione, mappaComuni, perc);
  		    
  			if(!mappaCandidati.isEmpty()) {
  			
  			cercaZeroPresenti(parziale, 0, mappaCandidati, L);
