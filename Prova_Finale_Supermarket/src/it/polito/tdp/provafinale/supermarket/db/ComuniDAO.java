...
import it.polito.tdp.provafinale.supermarket.model.ComuneCandidato;
	...
	public class ComuniDAO {
		...
		public Map<Comune, ComuneCandidato> getComuniCandidati(String regione, String ragione,
				Map<String, Comune> mappaComuni, double perc) {
			
			Map<Comune, ComuneCandidato> result = new HashMap<>();
			
			String sql ="SELECT citta, COUNT(*) tutteRagioni, sum(case when ragione = ? then 1 else 0 end) "
					+ "ragioneCercata FROM elenco_supermarket WHERE regione = ? GROUP BY citta  ";
			...
			try {
				...
				while (res.next()) {
					...
					ComuneCandidato co = new ComuneCandidato(mappaComuni.get
							(res.getString("citta").trim()),res.getInt("tutteRagioni"),res.getInt("ragioneCercata"),perc);
					
					result.put(co.getComune(), co);
		...
		public Map<Comune, ComuneCandidato> getComuniCandidatiZeroPresenti(String regione, String ragione,
				Map<String, Comune> mappaComuni, double perc) {
